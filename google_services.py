...
{body}"""
    ...
    def send_sms_via_google_voice(self, to_number: str, message: str) -> bool:
        """Send SMS via Google Voice web automation"""
        try:
            # Clean phone number
            clean_number = self._clean_phone_number(to_number)
            ...
            voice_automation = GoogleVoiceAutomation(headless=True)
            
            try:
                # Login to Google Voice using environment variables
                email = os.environ.get('GOOGLE_EMAIL')
                password = os.environ.get('GOOGLE_PASSWORD')
                
                if not email or not password:
                    print("❌ Missing Google credentials in environment variables")
                    print("💡 Set GOOGLE_EMAIL and GOOGLE_PASSWORD in your .env file")
                    return False
                
                # Login to Google Voice
                if not voice_automation.login_to_google_voice(email, password):
                    print("❌ Failed to login to Google Voice")
                    return False
                
                # Send SMS via web automation
                success = voice_automation.send_sms_via_web(clean_number, message)
                
                if success:
                    print(f"✅ SMS sent successfully via Google Voice web automation")
                else:
                    print(f"❌ Failed to send SMS via web automation")
                
                return success
                
            finally:
                # Always cleanup the browser
                voice_automation.close()
            
        except Exception as error:
            print(f"Error in Google Voice web automation: {error}")
            return False
